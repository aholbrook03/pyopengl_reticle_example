...
class Matrix4(object):
    ...
    @staticmethod
    def getOrthographic(top=1, bottom=-1, left=-1, right=1, near=1, far=2, aspect=1):
        a = 2 / ((right - left) * aspect)
    ...
    @staticmethod
    def getPerspective(fovy=60, near=0.001, far=10, aspect=1):
        b = 1.0 / tan(radians(fovy / 2))
        a = b / aspect
        c = -(far + near) / (far - near)
        d = -(2 * (near * far)) / (far - near)
        
        return Matrix4(((a, 0, 0, 0), (0, b, 0, 0), (0, 0, c, d), (0, 0, -1, 0)))
    ...
    def getTranspose(self):
        return Matrix4([[self.data[i][j] for i in range(4)] for j in range(4)])
    ...
    def basis(self):
        x = Vector4((self.get(0, 0), self.get(1, 0), self.get(2, 0), 0))
        y = Vector4((self.get(0, 1), self.get(1, 1), self.get(2, 1), 0))
        z = Vector4((self.get(0, 2), self.get(1, 2), self.get(2, 2), 0))
